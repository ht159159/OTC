<template src="./NumericKeypad.html"></template>
<script>
export default {
    data () {
        return {
            isPwdInput : {
                ...
                init: function (callback) {
                    ...
                    var html = '<div class="pwd_fixed" id="pwd_fixed"><div class="pwd_box">' + '<div class="pwd_title"><span class="pwd_close"></span><span>请输入安全码</span></div>' + '<div class="pwd_input pwd_line">' + temp + "</div>" + '<div class="pwd_num">' + temp2 + "</div>" + "</div></div>";
                    document.querySelector(".NumericKeypad").insertAdjacentHTML("beforeend", html);
                    setTimeout(function () {
                        document.querySelectorAll(".pwd_box")[0].classList.add("on")
                    }, 100)
    ...
    methods: {
        openkeyboard() {
            this.isPwdInput.open((a) =>{
                if(a.pwd){
                    if(a.pwd.length == 6){
                        this.$emit('safepwd', a.pwd)
                        this.$parent.submit();
                    }
                }
            });
...
<style lang="scss" src="./NumericKeypad.scss"></style>