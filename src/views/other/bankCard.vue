<template src="./bankCard.html"></template>
...
export default {
  ...
  created () {
    ...
    for (let i = 0; this.accountsList.data.length > i; i++) {
      if (this.accountsList.data[i].type === 1) {
        this.editShow = true
        this.bankCardList.push(this.accountsList.data[i])
      }
    }
  },
  methods: {
    edit (v) {
      ...
      this.$storage.session.set('editData', this.bankCardList)
    ...
    // },
    determine () {
      this.deleteData.id = this.bankCardList[this.deleteIndex].id
      this.bankCardList.splice(this.deleteIndex, 1)
...
<style lang="scss" src="./bankCard.scss" scoped>
